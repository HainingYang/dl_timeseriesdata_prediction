...
class OutliersHandler(Handler):
    class LSTMPredictor(object):
        ...
        def predict(self):
            ...
            y_pred = self._model.predict(X, batch_size=self._batch, verbose=1)
    ...
    def end_batch(self, end_req):
        ...
        predictions = self._predictor.predict() # dimension is (num of points) - sequence length
        logger.info('number of predictions: ' + str(len(predictions)))

        self._begin_response.begin.size = len(predictions)
        self._agent.write_response(self._begin_response)

        response = udf_pb2.Response()
        for i in range(self._sequence, len(self._points)):
            response.point.CopyFrom(self._points[i])
            #response.point.ClearField('fieldsDouble')
            response.point.fieldsDouble["predict"]=predictions[i-self._sequence]
